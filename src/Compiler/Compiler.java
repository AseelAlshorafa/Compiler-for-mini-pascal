...
public class Compiler {
    ...
    public Program compile(char[] input, PrintWriter outError) {
        ...
        if (error.wasAnErrorSignalled()) {
            ...
            System.out.println("compiled entire file successfully  both LEXIACL, syntax and semantic phase ended");
            return p;
        }
    }
    ...
    private void subDeclaration(ArrayList<Subroutine> subList) {
        ...
        /*
         * Symbol table now searches for an identifier in the scope order. First the
         * local variables and parameters and then the procedures and functions. at this
         * point, there should not be any local variables/parameters in the symbol
         * table.
         */
        ...
        if (isFunction) {
            // currentFunction is used to store the function being compiled or null if it is
            // a procedure
    ...
    private StatementList statementList() {
        ...
        while (lexer.token == Symbol.IDENT || lexer.token == Symbol.IF || lexer.token == Symbol.WHILE
                || lexer.token == Symbol.READ || lexer.token == Symbol.WRITE || lexer.token == Symbol.WRITELN
    ...
    // stmt ::= ifstmt | whilestmt | assignstmt | compstmt | readstmt | writestmt |
    // writelnstmt |
    ...
    private Variable variable() {
        String name = (String) lexer.getStringValue();
        /*
         * is the variable in the symbol table? Variables are inserted in the symbol
         * table when they are declared. It the variable is not there, it has not been
         * declared.
         */
    ...
    private Expr expr() {
        ...
        if (relOp == Symbol.EQ || relOp == Symbol.LT || relOp == Symbol.GT || relOp == Symbol.LE || relOp == Symbol.GE
                || relOp == Symbol.NEQ) {
            lexer.nextToken();
            right = expr();
    ...
    private Expr term() {
        ...
        while (multOp == Symbol.MULT || multOp == Symbol.REALDIV || multOp == Symbol.AND || multOp == Symbol.MOD
                || multOp == Symbol.DIV) {
            lexer.nextToken();
            right = factor();
