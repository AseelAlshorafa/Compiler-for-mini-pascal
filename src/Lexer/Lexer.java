...
public class Lexer {
    ...
    /**
     * Constructor. @input receives string input from @args[0] and @error receives
    ...
    public void skipBraces() {
        ...
        if (token == Symbol.LEFTCURLYBRACKET || token == Symbol.RIGHTCURLYBRACKET || token == Symbol.LEFTBRACKET
    ...
    public void skipToNextStatement() {
        while (token != Symbol.EOF && token != Symbol.ELSE && token != Symbol.ENDIF && token != Symbol.END
    ...
    public void printMethod(String value, int token, int line_no) {
        System.out.println(value + "\t" + token + "\t" + line_no);
